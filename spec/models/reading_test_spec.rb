...
RSpec.describe ReadingTest, :type => :model do
  describe "Generate test" do
    context "gap-fill test" do
      ...
      let(:short_text) { create(:text, content: '中国笨蛋', title: '') }
      let(:short_text_test) { create(:reading_test, text: short_text) }
      let(:text) { create(:text, content: "我是懒猪。你是铁公鸡。", title: "猪和公鸡") }
      let(:test) { create(:reading_test, text: text) }

      it "generates helper answers for the user" do
        test = create(:reading_test, text: text, data: {"answer" => "铁公鸡"})
        test.generate_helper_answers
        expect(test.data['helper_answers'].count).to eq 4
        expect(test.data['helper_answers']).to include "铁公鸡"
      end
